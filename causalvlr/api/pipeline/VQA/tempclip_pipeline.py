...
from causalvlr.utils.VQA import get_mask, load_file, calculate_IoU_batch
from tabulate import tabulate
import time
from causalvlr.utils.VQA import get_remain_time
...
class TempCLIPPipeline(BasePipeline):
    ...
    def eval(self, data_loader):
        ...
        with torch.no_grad():
            ...
            # ---------------------------------------
            score["acc"] /= count
            # Use dataset_root for JSON files (they are in the dataset root directory)
            dataset_root = data_loader.dataset.dataset_root
            gt_file = os.path.join(dataset_root, f'gsub_{data_loader.dataset.split}.json')
            gt_ground = load_file(gt_file)
            seg_file = os.path.join(dataset_root, f'frame2time_{data_loader.dataset.split}.json')
            segs = load_file(seg_file)
            # CSV files are in csv_path subdirectory
