...
import pandas as pd
import random
from causalvlr.utils.VQA import load_file, group, get_qsn_type, tokenize
from .vqa_prepare_video import video_sampling, prepare_input
...
class VideoQADataset(Dataset):
    def __init__(self, cfgs, split, tokenizer, a2id):
        ...
        # Determine dataset root and csv path
        csv_base_path = cfgs["dataset"]["csv_path"]
        # If csv_path ends with 'csv', it's pointing to a csv subdirectory
        if csv_base_path.endswith('csv') or csv_base_path.endswith('csv/'):
            self.dataset_root = osp.dirname(csv_base_path.rstrip('/'))
            csv_path = osp.join(csv_base_path, f"{split}.csv")
        else:
            # csv_path is the dataset root, csv files are directly there
            self.dataset_root = csv_base_path
            csv_path = osp.join(csv_base_path, f"{split}.csv")
        ...
        # NOTE gsub - use dataset root directory
        if split != "train":
            gsub_path = osp.join(self.dataset_root, f"gsub_{split}.json")
            self.gsub = load_file(gsub_path)
        else:
            self.gsub = None
        ...
        if self.mode not in ['val', 'test']:
            ...
            if self.agu:
                agu_file = osp.join(self.dataset_root, 'train_gpt4_sub.json')
                if not osp.exists(agu_file):
                    self.agu = False
                else:
